...
---
module: notebook
short_description: Manage RHOAI Jupyter notebooks
description:
    - Create, update, and delete notebook resources in Red Hat OpenShift AI.
...
options:
    ...
    namespace:
        description:
            - Namespace for the notebook.
        type: str
        required: true
    notebook_id:
        description:
            - ID of an existing notebook for update/delete.
        type: str
    image:
        description:
...
- name: Create a notebook
  stevefulme1.rhoai_mlops.notebook:
    api_url: "https://rhoai.example.com"
    api_token: "{ '{' } rhoai_token { '}' }"
    name: "fraud-analysis"
    namespace: "fraud-detection"
    image: "quay.io/rhoai/notebook:latest"
    state: "present"
"""

RETURN = r"""
notebook:
    description: Details of the notebook resource.
    returned: On success.
    type: dict
"""
...
def main():
    ...
    existing = None
    if params.get("notebook_id"):
        response = call_with_retry(client.get, f"/apis/v1/notebooks/{params['notebook_id']}")
    ...
    if state == "absent":
        ...
        response = call_with_retry(client.delete, f"/apis/v1/notebooks/{resource_id}")
        if response.status_code not in (200, 204, 404):
            module.fail_json(msg=f"Failed to delete notebook: {response.text}")
        module.exit_json(changed=True)
        return
    ...
    if existing is None:
        if module.check_mode:
            module.exit_json(changed=True)
        response = call_with_retry(client.post, "/apis/v1/notebooks", json=payload)
        if response.status_code not in (200, 201):
            module.fail_json(msg=f"Failed to create notebook: {response.text}")
        result = response.json()
        if params.get("wait") and result.get("id"):
            result = wait_for_resource(
                module,
                lambda rid: call_with_retry(client.get, f"/apis/v1/notebooks/{rid}").json(),
                result["id"],
                target_states=READY_STATES,
            )
        module.exit_json(changed=True, notebook=to_dict(result))
        return
    ...
    if not changed:
        module.exit_json(changed=False, notebook=to_dict(existing))
        return
    ...
    resource_id = existing.get("id", existing.get("name"))
    response = call_with_retry(client.patch, f"/apis/v1/notebooks/{resource_id}", json=payload)
    if response.status_code not in (200, 202):
        module.fail_json(msg=f"Failed to update notebook: {response.text}")
