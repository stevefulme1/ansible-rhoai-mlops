...
short_description: Manage RHOAI data science projects
description:
    - Create, update, and delete data science project resources in Red Hat OpenShift AI.
...
options:
    name:
        description:
            - Name of the data science project.
        type: str
        required: true
    project_id:
        description:
            - ID of an existing project for update/delete.
        type: str
    description:
        description:
...
EXAMPLES = r"""
- name: Create a data science project
  stevefulme1.rhoai_mlops.data_science_project:
    api_url: "https://rhoai.example.com"
    api_token: "{ '{' } rhoai_token { '}' }"
    name: "fraud-detection"
    description: "Fraud detection ML project"
    state: "present"
"""

RETURN = r"""
data_science_project:
    description: Details of the data science project resource.
    returned: On success.
    type: dict
"""
...
def main():
    ...
    if state == "absent":
        ...
        response = call_with_retry(client.delete, f"/apis/v1/namespaces/{resource_id}")
        if response.status_code not in (200, 204, 404):
            module.fail_json(msg=f"Failed to delete data science project: {response.text}")
        module.exit_json(changed=True)
        return
    ...
    if existing is None:
        if module.check_mode:
            module.exit_json(changed=True)
        response = call_with_retry(client.post, "/apis/v1/namespaces", json=payload)
        if response.status_code not in (200, 201):
            module.fail_json(msg=f"Failed to create data science project: {response.text}")
        result = response.json()
        if params.get("wait") and result.get("id"):
            result = wait_for_resource(
                module,
                lambda rid: call_with_retry(client.get, f"/apis/v1/namespaces/{rid}").json(),
                result["id"],
                target_states=READY_STATES,
            )
        module.exit_json(changed=True, data_science_project=to_dict(result))
        return
    ...
    if not changed:
        module.exit_json(changed=False, data_science_project=to_dict(existing))
        return
    ...
    resource_id = existing.get("id", existing.get("name"))
    response = call_with_retry(client.patch, f"/apis/v1/namespaces/{resource_id}", json=payload)
    if response.status_code not in (200, 202):
        module.fail_json(msg=f"Failed to update data science project: {response.text}")
