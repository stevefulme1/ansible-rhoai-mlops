...
"""Ansible module for managing RHOAI pipeline resources."""
...
---
module: pipeline
short_description: Manage RHOAI ML pipelines
description:
    - Create, update, and delete pipeline resources in Red Hat OpenShift AI.
...
options:
    ...
    pipeline_id:
        description:
            - ID of an existing pipeline for update/delete.
        type: str
    description:
        description:
    ...
    pipeline_spec:
        description:
            - Pipeline specification (Kubeflow Pipelines v2 format).
        type: dict
    state:
        description:
            - Desired state.
...
- name: Create a pipeline
  stevefulme1.rhoai_mlops.pipeline:
...
RETURN = r"""
pipeline:
    description: Details of the pipeline resource.
    returned: On success.
    type: dict
"""
...
def main():
    ...
    if state == "absent":
        ...
        response = call_with_retry(client.delete, f"/apis/v2beta1/pipelines/{resource_id}")
        if response.status_code not in (200, 204, 404):
            module.fail_json(msg=f"Failed to delete pipeline: {response.text}")
        module.exit_json(changed=True)
        return
    ...
    if existing is None:
        if module.check_mode:
            module.exit_json(changed=True)
        response = call_with_retry(client.post, "/apis/v2beta1/pipelines", json=payload)
        if response.status_code not in (200, 201):
            module.fail_json(msg=f"Failed to create pipeline: {response.text}")
        result = response.json()
        if params.get("wait") and result.get("id"):
            result = wait_for_resource(
                module,
                lambda rid: call_with_retry(client.get, f"/apis/v2beta1/pipelines/{rid}").json(),
                result["id"],
                target_states=READY_STATES,
            )
        module.exit_json(changed=True, pipeline=to_dict(result))
        return
    ...
    if not changed:
        module.exit_json(changed=False, pipeline=to_dict(existing))
        return
    ...
    resource_id = existing.get("id", existing.get("name"))
    response = call_with_retry(client.patch, f"/apis/v2beta1/pipelines/{resource_id}", json=payload)
    if response.status_code not in (200, 202):
        module.fail_json(msg=f"Failed to update pipeline: {response.text}")
