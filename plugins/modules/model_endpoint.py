...
short_description: Manage RHOAI model serving endpoints
description:
    - Create, update, and delete model endpoint resources in Red Hat OpenShift AI.
...
options:
    ...
    endpoint_id:
        description:
            - ID of an existing endpoint for update/delete.
        type: str
    inference_service:
        description:
            - Name of the InferenceService to expose.
        type: str
    traffic_split:
        description:
...
RETURN = r"""
model_endpoint:
    description: Details of the model endpoint resource.
    returned: On success.
    type: dict
"""
...
def get_module_args():
    module_args = dict(
        ...
        inference_service=dict(type="str"),
        traffic_split=dict(type="dict"),
...
def main():
    ...
    if params.get("endpoint_id"):
        response = call_with_retry(client.get, f"/apis/v1beta1/endpoints/{params['endpoint_id']}")
    ...
    if state == "absent":
        ...
        response = call_with_retry(client.delete, f"/apis/v1beta1/endpoints/{resource_id}")
        if response.status_code not in (200, 204, 404):
            module.fail_json(msg=f"Failed to delete model endpoint: {response.text}")
        module.exit_json(changed=True)
        return
    ...
    if existing is None:
        if module.check_mode:
            module.exit_json(changed=True)
        response = call_with_retry(client.post, "/apis/v1beta1/endpoints", json=payload)
        if response.status_code not in (200, 201):
            module.fail_json(msg=f"Failed to create model endpoint: {response.text}")
        result = response.json()
        if params.get("wait") and result.get("id"):
            result = wait_for_resource(
                module,
                lambda rid: call_with_retry(client.get, f"/apis/v1beta1/endpoints/{rid}").json(),
                result["id"],
                target_states=READY_STATES,
            )
        module.exit_json(changed=True, model_endpoint=to_dict(result))
        return
    ...
    if not changed:
        module.exit_json(changed=False, model_endpoint=to_dict(existing))
        return
    ...
    resource_id = existing.get("id", existing.get("name"))
    response = call_with_retry(client.patch, f"/apis/v1beta1/endpoints/{resource_id}", json=payload)
    if response.status_code not in (200, 202):
        module.fail_json(msg=f"Failed to update model endpoint: {response.text}")
